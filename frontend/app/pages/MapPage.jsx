...
const DisneylandMap = () => {
  ...
  const getApiUrl = () => "http://172.20.10.2:3000/api/locations";
  ...
  const fetchWaitTimes = async () => {
    try {
      ...
      console.error(
        "Erreur lors de la récupération des temps d'attente:",
        error
      );
    ...
    try {
      ...
      setAttractions((prevAttractions) => {
        return prevAttractions.map((attraction, index) => {
          const dbAttraction = response.data.find(
      ...
      console.error(
        "❌ Erreur lors de la récupération des attractions depuis le backend :",
        error
      );
